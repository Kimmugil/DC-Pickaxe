...
def backfill_gaps(gallery_id, gap_dates, existing_ids, gallery_type):
    """
    gap_dates 기간의 누락 게시글 메타데이터를 수집 (Phase 1).
    본문은 fetch_contents_async로 별도 수집 (Phase 2).
    """
    if not gap_dates:
        return []
        ...
        try:
            ...
            for row in rows:
                ...
                meta_list.append({
                    ...
                    "writer": writer, "date": date_val, "link": post_link,
                    "cc": cc, "vc": vc, "rc": rc,
                })
            ...
            if page_new:
                print(f"  [{gallery_id}] 페이지 {page}: {page_new}건 수집 (누적 {len(meta_list)}건)")
    ...
    # Phase 2: 본문 async 병렬 수집
    print(f"  [{gallery_id}] 본문 수집 중 ({len(meta_list)}건)...")
    contents = asyncio.run(
        fetch_contents_async(gallery_id, url_prefix, [m["id"] for m in meta_list])
    )

    all_new = []
    for m in meta_list:
        all_new.append([
            m["id"], m["title"], contents.get(m["id"], ""),
            m["writer"], m["date"], m["link"], m["cc"], m["vc"], m["rc"],
        ])
    return all_new
