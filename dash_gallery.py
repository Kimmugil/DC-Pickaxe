...
def render(row, nc, ic, uc, rc, lc, cfg):
    ...
    for col, icon, val, lbl in [
        (m1, "📄", f"{total:,}건",             cfg.get("kpi_gall_total", "총 수집 게시글")),
        (m2, "📅", str(dmin) if dmin else "-", cfg.get("kpi_gall_first", "최초 수집")),
        (m3, "🗓️", str(dmax) if dmax else "-", cfg.get("kpi_gall_recent", "최근 수집")),
        (m4, "📈", f"{avg}건/일",              cfg.get("kpi_gall_avg", "일평균")),
    ]:
        with col:
            st.markdown(
    ...
    with hot_col:
        ...
        st.markdown(
            ...
            f"<p class='sub' style='margin:0 0 10px'>{period} 기준</p>",
            unsafe_allow_html=True,
        )
        if hot.empty:
    ...
    with line_col:
        ...
        st.markdown(
            ...
            f"<p class='ctitle' style='margin:0'>{cfg.get('title_cumul', '📈 누적 추이')}</p>"
    ...
    with tbl_hdr:
        st.markdown(
            f"<p class='sec'>{cfg.get('title_recent_list', '📝 최근 수집 게시글')} (최신 50건)</p>",
            unsafe_allow_html=True,
        )
    with tbl_btn:
